...
class FlockMCPResourceBase(BaseModel):
    """Represents a Resource retrieved from a server.
    ...
    mime_type: str | None = Field(
        default=None, description="(Optional) Mime-Type of the Resource."
    )
    ...
    @classmethod
    def try_to_mcp_resource(
        cls: type[T], resource: T
    ) -> tuple[Resource | None, ReadResourceResult | None] | None:
        ...
        try:
            if resource.uri:
                ...
                if (
                    hasattr(resource, "data_type")
                    and resource.data_type
                    and resource.data_type == "base64"
                ) or resource.data_type == "text":
                    data_result = None
                    ...
                    if resource.data_type == "base64":
                        ...
                        data_result = ReadResourceResult(contents=data_contents)
    ...
    @classmethod
    def try_from_mcp_resource(
        cls: type[T],
        resource: Resource,
        contents: ReadResourceResult | None = None,
    ) -> type[T] | None:
                ...
                if contents:
                    ...
                    if isinstance(data_contents, list[BlobResourceContents]):
                        ...
                        for blob_content in data_contents:
                            if (
                                hasattr(blob_content, "blob")
                                and blob_content.blob
                            ):
