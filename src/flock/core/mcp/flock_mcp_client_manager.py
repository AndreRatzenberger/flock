...
import random
from typing import Annotated, Any, Callable, Generic, Literal, Type, TypeVar
...
class FlockMCPClientManagerConfigBase(BaseModel):
    ...
    mount_points: list[Root] | None = Field(
        default=None,
        description="Initial Mounting Points on the server."
    )

    tool_cache_max_size: float = Field(
        default=100,
        description="Max number of items in the tools cache."
    )
