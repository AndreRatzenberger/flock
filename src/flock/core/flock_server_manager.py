...
class FlockServerManager(BaseModel):
    ...
    servers: List[FlockMCPServerBase] | None = Field(
        ...,
        exclude=True,
        description="The servers to manage."
    )
    ...
    lock: Lock | None = Field(
        default=None,
        exclude=True,
        description="Global lock for mutex access."
    )
    ...
    def __init__(self, servers: List[FlockMCPServerBase] | None = None, stack: AsyncExitStack | None = None, lock: asyncio.Lock | None = None) -> None:
        super().__init__(
            servers=servers,
            stack=stack,
            lock=lock,
    ...
    async def add_server_during_runtime(self, server: FlockMCPServerBase) -> None:
    ...
    async def remove_server_during_runtime(self, server: FlockMCPServerBase) -> None:
